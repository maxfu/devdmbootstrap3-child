<?php
/**
 * Generated by the WordPress Meta Box Generator at http://goo.gl/8nwllb
 */
 // Add Macland Options to Admin Menu.
 $pages = array(
   'maxwell_options'   => array(
     'page_title'    => __( 'Maxwell Options', 'devdmbootstrap3' ),
     'position'      => 4,
     'sections'      => array(
       'section-one'   => array(
         'title'         => __( 'Layout Options', 'devdmbootstrap3' ),
         'fields'        => array(
           'main_fluid' => array(
             'title'         => __( 'Fluid Main Container', 'devdmbootstrap3' ),
             'id'            => 'main_fluid',
             'type'          => 'checkbox',
             'text'          => __( 'Use full-width fluid containers on main content.', 'devdmbootstrap3' ),
           ),
           'header_fluid' => array(
             'title'         => __( 'Fluid Header Container', 'devdmbootstrap3' ),
             'id'            => 'header_fluid',
             'type'          => 'checkbox',
             'text'          => __( 'Use full-width fluid containers on header.', 'devdmbootstrap3' ),
           ),
           'footer_fluid' => array(
             'title'         => __( 'Fluid Footer Container', 'devdmbootstrap3' ),
             'id'            => 'footer_fluid',
             'type'          => 'checkbox',
             'text'          => __( 'Use full-width fluid containers on footer.', 'devdmbootstrap3' ),
           ),
           'bootstrap_theme' => array(
             'title'         => __( 'Bootstrap Theme', 'devdmbootstrap3' ),
             'id'            => 'bootstrap_theme',
             'type'          => 'radio',
             'choices'       => array(
               'default'     => __( 'Default theme, Light gray', 'devdmbootstrap3' ),
               'cerulean'    => __( 'Cerulean, A calm blue sky', 'devdmbootstrap3' ),
               'cosmo'       => __( 'Cosmo, An ode to Metro', 'devdmbootstrap3' ),
               'cyborg'      => __( 'Cyborg, Jet black and electric blue', 'devdmbootstrap3' ),
               'darkly'      => __( 'Darkly, Flatly in night mode', 'devdmbootstrap3' ),
               'flatly'      => __( 'Flatly, Flat and modern', 'devdmbootstrap3' ),
               'journal'     => __( 'Journal, Crisp like a new sheet of paper', 'devdmbootstrap3' ),
               'lumen'       => __( 'Lumen, Light and shadow', 'devdmbootstrap3' ),
               'paper'       => __( 'Paper, Material is the metaphor', 'devdmbootstrap3' ),
               'readable'    => __( 'Readable, Optimized for legibility', 'devdmbootstrap3' ),
               'sandstone'   => __( 'Sandstone, A touch of warmth', 'devdmbootstrap3' ),
               'simplex'     => __( 'Simplex, Mini and minimalist', 'devdmbootstrap3' ),
               'slate'       => __( 'Slate, Shades of gunmetal gray', 'devdmbootstrap3' ),
               'spacelab'    => __( 'Spacelab, Silvery and sleek', 'devdmbootstrap3' ),
               'superhero'   => __( 'Superhero, The brave and the blue', 'devdmbootstrap3' ),
               'united'      => __( 'United, Ubuntu orange and unique font', 'devdmbootstrap3' ),
               'yeti'        => __( 'Yeti, A friendly foundation', 'devdmbootstrap3' ),
             ),
           ),
           'navbar_style'    => array(
             'title'         => __( 'Navbar Style', 'devdmbootstrap3' ),
             'id'            => 'navbar_style',
             'type'          => 'radio',
             'choices'       => array(
               'default'     => __( 'Default style. Choose a style ... ', 'devdmbootstrap3' ),
               'inverse'     => __( 'Inverse style. matches your design.', 'devdmbootstrap3' ),
             ),
           ),
           'menu_search'    => array(
             'title'         => __( 'Search Menu Item' ),
             'id'            => 'menu_search',
             'type'          => 'checkbox',
             'text'          => __( 'Display Search on Main Menu. If thrid party menu search plugin installed, uncheck this' ),
           ),
         ),
       ),
       'section-two'   => array(
         'title'         => __( 'Website Options', 'devdmbootstrap3' ),
         'fields'        => array(
           'seo_keywords'    => array(
             'title'         => __( 'SEO Keywords', 'devdmbootstrap3' ),
             'id'            => 'seo_keywords',
             'type'          => 'text',
             'placeholder'   => 'Use comma to seprate.',
           ),
//           'act_pic'         => array(
//             'title'         => __( 'Activity Picture' ),
//             'id'            => 'activity_pic',
//             'type'          => 'media',
//             'value'         => '',
//             'placeholder'   => 'Click Upload to upload or choose picture for Activity',
//           ),
//           'act_date'        => array(
//             'title'         => __( 'Activity Date' ),
//             'id'            => 'activity_date',
//             'type'          => 'date',
//             'value'         => 'now',
//           ),
//           'h_slider_1'      => array(
//             'title'         => __( 'Home Slider Picrure 1' ),
//             'id'            => 'home_slider_1',
//             'type'          => 'media',
//             'value'         => '',
//             'placeholder'   => 'Click Upload to upload or choose the picture.',
//           ),
//           'h_sl_link_1'     => array(
//             'title'         => __( 'Home Slider Link 1' ),
//             'id'            => 'home_slider_link_1',
//             'type'          => 'url',
//             'placeholder'   => 'Link for Home Slider 1',
//           ),
//           'h_slider_2'      => array(
//             'title'         => __( 'Home Slider Picrure 2' ),
//             'id'            => 'home_slider_2',
//             'type'          => 'media',
//             'value'         => '',
//             'placeholder'   => 'Click Upload to upload or choose the picture.',
//           ),
//           'h_sl_link_2'     => array(
//             'title'         => __( 'Home Slider Link 2' ),
//             'id'            => 'home_slider_link_2',
//             'type'          => 'url',
//             'placeholder'   => 'Link for Home Slider 2',
//           ),
//           'h_slider_3'      => array(
//             'title'         => __( 'Home Slider Picrure 3' ),
//             'id'            => 'home_slider_3',
//             'type'          => 'media',
//             'value'         => '',
//             'placeholder'   => 'Click Upload to upload or choose the picture.',
//           ),
//           'h_sl_link_3'     => array(
//             'title'         => __( 'Home Slider Link 3' ),
//             'id'            => 'home_slider_link_3',
//             'type'          => 'url',
//             'placeholder'   => 'Link for Home Slider 3',
//           ),
//           'disp_contact'    => array(
//             'title'         => __( 'Staff Contact Information' ),
//             'id'            => 'display_contact',
//             'type'          => 'checkbox',
//             'text'          => __( 'Display contact information in Team Page' ),
//           ),
         ),
       ),
       'section-three'   => array(
         'title'         => __( 'Business Options', 'devdmbootstrap3' ),
         'fields'        => array(
           'comp_tel'        => array(
             'title'         => __( 'Company Telephone', 'devdmbootstrap3' ),
             'id'            => 'company_tel',
             'type'          => 'tel',
             'placeholder'   => '61-412-901-396',
           ),
           'comp_fax'        => array(
             'title'         => __( 'Company Fax' ),
             'id'            => 'company_fax',
             'type'          => 'tel',
             'placeholder'   => '61-2-9736-2681',
           ),
           'comp_email'        => array(
             'title'         => __( 'Company Telephone', 'devdmbootstrap3' ),
             'id'            => 'company_email',
             'type'          => 'email',
             'placeholder'   => 'enquiry@maxwellit.com.au',
           ),
           'comp_address'    => array(
             'title'         => __( 'Company Address', 'devdmbootstrap3' ),
             'id'            => 'company_address',
             'type'          => 'text',
             'placeholder'   => '32/29-31 Paul St Bondi Junction NSW 2022',
           ),
           'comp_s_weibo'    => array(
             'title'         => __( 'Sina Weibo', 'devdmbootstrap3' ),
             'id'            => 'company_sina_weibo',
             'type'          => 'url',
             'placeholder'   => __( 'Link of Sina Weibo', 'devdmbootstrap3' ),
           ),
           'comp_t_weibo'    => array(
             'title'         => __( 'Tencent Weibo', 'devdmbootstrap3' ),
             'id'            => 'company_tencent_weibo',
             'type'          => 'url',
             'placeholder'   => __( 'Link of Tencent Weibo', 'devdmbootstrap3' ),
           ),
           'comp_facebook'   => array(
             'title'         => __( 'Facebook', 'devdmbootstrap3' ),
             'id'            => 'company_facebook',
             'type'          => 'url',
             'placeholder'   => __( 'Link of Facebook', 'devdmbootstrap3' ),
           ),
           'comp_googleplus' => array(
             'title'         => __( 'Google+', 'devdmbootstrap3' ),
             'id'            => 'company_googleplus',
             'type'          => 'url',
             'placeholder'   => __( 'Link of Google+', 'devdmbootstrap3' ),
           ),
           'comp_twitter'    => array(
             'title'         => __( 'Twitter', 'devdmbootstrap3' ),
             'id'            => 'company_twitter',
             'type'          => 'url',
             'placeholder'   => __( 'Link of Twitter', 'devdmbootstrap3' ),
           ),
           'comp_linkedin'     => array(
             'title'         => __( 'Linkedin', 'devdmbootstrap3' ),
             'id'            => 'company_linkedin',
             'type'          => 'url',
             'placeholder'   => __( 'Link of Linkedin', 'devdmbootstrap3' ),
           ),
           'comp_pinterest'     => array(
             'title'         => __( 'Pinterest', 'devdmbootstrap3' ),
             'id'            => 'company_pinterest',
             'type'          => 'url',
             'placeholder'   => __( 'Link of Pinterest', 'devdmbootstrap3' ),
           ),
           'comp_wechat'     => array(
             'title'         => __( 'WeChat QrCode' ),
             'id'            => 'company_wechat',
             'type'          => 'media',
             'value'         => '',
             'placeholder'   => __( 'Click Upload to upload or choose the QrCode.', 'devdmbootstrap3' ),
           ),
         ),
       ),
     ),
   ),
 );
 $option_page = new RationalOptionPages( $pages );
?>
